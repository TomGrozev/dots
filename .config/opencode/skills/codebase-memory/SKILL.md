...
## Sessions and Tiers
- At session start or after compaction, call `list_projects`/`index_status` before structural exploration to confirm the project is indexed and fresh.
- Choose a tier to match the task: Scout (quick provisional), Verify (default), Auditor (exhaustive). The tier governs how many graph calls to make and how much coverage to verify — it is a self-discipline for the agent running the queries, not a separate agent.
- A child agent without MCP tools (delegated brief) must not call or claim MCP access. It works from supplied evidence and uses read/grep on exact source, especially every reported missed-coverage range.
